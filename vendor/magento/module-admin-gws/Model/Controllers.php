...
class Controllers extends \Magento\AdminGws\Model\Observer\AbstractObserver implements CallbackProcessorInterface
{
        ...
        if ($this->_role->getWebsiteIds()) {
            $this->_redirect(
                $this->_backendUrl->getUrl(
                    ...
                    ['website' => $this->getAnyStoreView()->getWebsite()->getId()]
                )
            );
            return;
        }
    ...
    protected function getAnyStoreView()
    {
        ...
        if (!$store || ($this->_role->getStoreIds() && !in_array($store->getId(), $this->_role->getStoreIds()))) {
            $allowedStoreIds = $this->_role->getStoreIds();
            if ($allowedStoreIds) {
                $store = $this->_storeManager->getStore(current($allowedStoreIds));
            }
        }
        if ($store) {
            return $store;
